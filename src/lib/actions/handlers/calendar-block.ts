...
export async function handleCalendarBlock(action: PAAction): Promise<ExecutionResult> {
  ...
  try {
    const event = google
      ? await googleCalendar.createEvent(action.userId, action.orgId, {
          summary: payload.title ?? "Focus Time",
    ...
    return { success: false, error: err instanceof Error ? err.message : "Failed to create calendar block" };
  }
}
